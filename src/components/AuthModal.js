...
export class AuthModal extends Modal {
    ...
    _bindEvents() {
        ...
        if (toggleBtn) {
            toggleBtn.onclick = (e) => {
                e.preventDefault();
                this.setMode(this.mode === 'signup' ? 'login' : 'signup');
            };
        }
        ...
        return `
            <div class="flex flex-col h-full px-8 py-12 text-center bg-dark-bg">
                ...
                <footer class="w-full flex flex-col items-center pb-8">
                    <button id="google-auth-btn" class="w-full bg-white text-black py-4 px-6 rounded-[24px] flex items-center justify-center gap-3 active:scale-[0.98] transition-all mb-8 shadow-lg shadow-white/5">
                        ...
                        <span class="text-base font-bold">Google 계정으로 회원가입</span>
                    </button>
                    <div class="mb-10">
                        <button id="auth-toggle" class="text-muted-lavender text-sm font-medium hover:text-white transition-colors">
                            이미 계정이 있나요? <span class="underline underline-offset-4 decoration-primary/40 font-bold text-primary">로그인하기</span>
                        </button>
                    </div>
                    <p class="text-[11px] text-muted-lavender/40 text-center leading-relaxed">
                        가입 시 <span class="underline decoration-muted-lavender/30">이용약관</span> 및 <span class="underline decoration-muted-lavender/30">개인정보처리방침</span>에 동의하게 됩니다.
                    </p>
                </footer>
            </div>
        `;
    ...
    _getLoginHTML() {
        return `
            <div class="flex flex-col h-full px-8 py-12 text-center bg-dark-bg">
                ...
                <main class="flex-1 flex flex-col items-center">
                    <h2 class="text-2xl font-bold leading-relaxed mb-4">
                        다시 만나서 반가워요.
                    </h2>
                    <p class="text-muted-lavender/60 text-base">
                        오늘의 기억을 정리할 준비가 되셨나요?
                    </p>
                </main>
                <footer class="w-full flex flex-col items-center pb-8">
                    <button id="google-auth-btn" class="w-full bg-white text-black py-4 px-6 rounded-[24px] flex items-center justify-center gap-3 active:scale-[0.98] transition-all mb-8 shadow-lg shadow-white/5">
                        ...
                        <span class="text-base font-bold">Google 계정으로 로그인</span>
                    </button>
                    <div class="mb-10">
                        <button id="auth-toggle" class="text-muted-lavender text-sm font-medium hover:text-white transition-colors">
                            처음이신가요? <span class="text-primary font-medium underline underline-offset-4 decoration-primary/30">회원가입하기</span>
                        </button>
                    </div>
                    <p class="text-[11px] text-muted-lavender/40 text-center leading-relaxed">
                        로그인 시 <span class="underline decoration-muted-lavender/30">이용약관</span> 및 <span class="underline decoration-muted-lavender/30">개인정보처리방침</span>에 동의하게 됩니다.
                    </p>
                </footer>
            </div>
        `;
